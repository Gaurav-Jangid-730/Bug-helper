...
def subdomain_finding(target, target_dir, enable_bruteforce):
    ...
    commands.extend([
        f"cat {subdomains} {target_dir}/sub-{target}.txt {target_dir}/asset-{target}.txt {target_dir}/sub-find-{target}.txt {target_dir}/findomain-{target}.txt {subdomains2} | sort -u > {target_dir}/final-{target}-subdomains.txt",
        ...
        f"rm {subdomains} {target_dir}/subdomains-list.txt {target_dir}/sub-{target}.txt {target_dir}/asset-{target}.txt {target_dir}/sub-find-{target}.txt {target_dir}/findomain-{target}.txt {subdomains2} {target_dir}/final-{target}-subdomains.txt"
    ])
